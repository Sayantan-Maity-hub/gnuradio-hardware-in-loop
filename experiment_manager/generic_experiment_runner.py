...
def run_generic_experiment(experiment_name, job_id, pr_id, parameter):
    ...
    if len(available_nodes) < required_nodes:
        ...
        raise RuntimeError(f"Experiment requires {required_nodes} nodes, but only {len(available_nodes)} are available")
    ...
    try:
        ...
        result = start_experiment(experiment_id=experiment_id, job_id=job_id,)
