...
from cortexlab.reservation.reservation_registry import get_all_reservation, get_reservation
    ...
    try:
        ...
        if current_reservation_job_id is not None:
            reservation = get_reservation(
                current_reservation_job_id
            )

            # Reservation finished/expired
            if reservation is not None:

                state = reservation.get(
                    "state", ""
                ).lower()
        ...
        result = run_generic_experiment( experiment_name=experiment_name, job_id=job_id, pr_id=pr_id, parameter=parameter)
...
def reservation_status_by_id(job_id):
    ...
    if reservation is None:
        return jsonify({
            "success": False,
            "message": f"Reservation {job_id} not found"
        }), 404
...
def node_status():
    ...
    return jsonify({"success": True, "count": len(nodes_status), "data": nodes_status}), 200
